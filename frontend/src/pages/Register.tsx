...
export function Register() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      ...
      <h2 className="font-heading text-2xl font-bold text-surface-100">Create your account</h2>
      <p className="mt-1 text-sm text-surface-400">Start your writing journey today</p>

      {error && <p className="mt-4 text-sm text-red-500 bg-red-500/10 p-3 rounded-lg">{error}</p>}

      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        <Input label="Full name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Alex Morgan" icon={<User size={18} />} required autoComplete="name" />
        ...
        <div className="relative">
          <Input label="Password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 8 characters" icon={<Lock size={18} />} required autoComplete="new-password" />
          <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-[38px] text-surface-500 hover:text-surface-300 transition-colors cursor-pointer" aria-label={showPassword ? 'Hide password' : 'Show password'}>
