...
import { Image, Save, Send, Eye, Bold, Italic, Code, List, Heading, Link2, Quote } from 'lucide-react';
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-heading text-3xl font-bold text-surface-100">Create Post</h1>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm"><Eye size={16} /> Preview</Button>
          <Button variant="secondary" size="sm"><Save size={16} /> Save Draft</Button>
          <Button size="sm"><Send size={16} /> Publish</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {/* Cover image upload */}
          <div className="glass rounded-2xl border-2 border-dashed border-surface-700 hover:border-primary-500/50 transition-colors p-10 text-center cursor-pointer group">
            <Image size={32} className="mx-auto text-surface-500 group-hover:text-primary-400 transition-colors" />
            <p className="mt-2 text-sm text-surface-400">Click or drag to upload cover image</p>
            <p className="text-xs text-surface-600 mt-1">Recommended: 1600×800px</p>
          </div>
